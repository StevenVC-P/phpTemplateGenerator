#!/usr/bin/env python3
"""
Test template_engineer with correct file structure
"""
...
async def test_template_engineer_final():
    """Test template_engineer with the latest files"""
    print("🔧 Testing Template Engineer (Final)")
    print("=" * 40)
    
    # Use the latest pipeline from test_chain.py
    pipeline_id = "pipeline_6d9df784"
    
    # Check if files exist
    spec_file = f"template_generations/template_6d9df784/specs/template_spec.json"
    design_file = f"template_generations/template_6d9df784/design_variations/design_variation_variation_20250624144211.json"
    prompt_file = f"template_generations/template_6d9df784/prompts/prompt_6d9df784.json"
    ...
    pipeline.pipeline_state[pipeline_id] = {
        ...
        'start_time': '2025-06-24T14:42:00',
        'status': 'running',
        'template_dir': f'template_generations/template_6d9df784'
    }
    ...
    try:
        ...
        if result.success:
            ...
            if result.output_file and Path(result.output_file).exists():
                ...
                content = Path(result.output_file).read_text()[:300]
...
if __name__ == "__main__":
    asyncio.run(test_template_engineer_final())
